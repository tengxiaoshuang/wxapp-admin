<template>
  <el-dialog :visible.sync="visible" :show-close="false" width="90%" :modal="true" :close-on-click-modal="false" :close-on-press-escape="false">
    ...
    <el-table key="listTable" border :data="formData" fit highlight-current-row style="width: 100%;margin-left:20px">
      ...
      <el-table-column label="BP">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <el-image :src="scope.row.bPFiles"></el-image>
            <div slot="reference" class="name-wrapper"><el-tag size="medium">BP</el-tag></div>
          </el-popover>
        </template>
      </el-table-column>
    ...
    <el-table border :data="data.listParams" style="width: 100%;margin-left:20px">
    ...
    <el-table border :data="data.ownershipList" style="width: 100%;margin-left:20px">
    ...
    <el-table border :data="data.financeList" style="width: 100%;margin-left:20px">
      ...
      this.$nextTick(() => {
        if (Id) {
          getListById(Id).then(data => {
            ...
            this.data.ownershipList = data.data.ownershipList.map(x => ({
              ...x,
              currentDate: moment(x.currentDate).format('YYYY-MM-DD')
            }));
            this.data.listParams = data.data.listParams.map(x => ({
              ...x,
