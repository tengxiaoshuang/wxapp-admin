  ...
  <el-dialog :visible.sync="visible" :show-close="false" width="800px" :modal="true" :close-on-click-modal="false" :close-on-press-escape="false">
    ...
    <el-form :model="formData3" :inline="true" ref="formData3" label-width="110px" size="medium">
      <el-row style="border-bottom: 1px solid #f0f0f0;padding: 10px;">
        ...
        <el-form-item label="注册时间" :prop="'registrationDate'" :rules="[{ required: true, message: '时间不能为空', trigger: 'change' }]">
          <el-date-picker v-model="formData3.registrationDate" type="date" ></el-date-picker>
        </el-form-item>
      ...
      <el-row v-for="(item, index) in formData3.financeList" :key="index + '#'" style="border-bottom: 1px solid #f0f0f0;padding: 10px;">
        ...
        <el-form-item label="融资时间" :prop="'financeList.' + index + '.financeTime'" :rules="[{ required: true, message: '时间不能为空', trigger: 'change' }]">
          <el-date-picker v-model="item.financeTime" type="date"></el-date-picker>
        </el-form-item>
    ...
    <el-row v-for="(item, index) in formData3.win" :key="index + '?'" style="border-bottom: 1px solid #f0f0f0;padding: 10px;">
      ...
      <el-form-item label="获奖时间" :prop="'win.' + index + '.awardTime'">
        <el-date-picker v-model="item.awardTime" type="date"></el-date-picker>
      </el-form-item>
      <el-button type="danger" v-if="formData3.win.length > 1" size="medium" @click="removeRow(index, 'win',item.id)">删除</el-button>
    </el-row>
...
import { enterprisesUpdate, getEnterprisesById } from '@/api/entrepreneur';
...
export default {
  ...
  methods: {
    ...
    // 删除属性列
    removeRow(index, name, id) {
      if (name == 'listParams') {
        this.formData3.listParams.splice(index, 1);
        if(id){
          this.dataArr.listParamsArr.push(id)
          Array.from(new Set(this.dataArr.listParamsArr));
        }
      }else if (name == 'win') {
        this.formData3.win.splice(index, 1);
        if(id){
          this.dataArr.winArr.push(id)
          Array.from(new Set(this.dataArr.winArr));
        }
      }else {
        this.formData3.financeList.splice(index, 1);
        if(id){
          this.dataArr.financeListArr.push(id)
          Array.from(new Set(this.dataArr.financeListArr));
        }
      }
    },
    // 提交
    submit(formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          this.formData3.dataArr=this.dataArr;
          console.log(this.formData3)
          enterprisesUpdate(this.formData3).then(res => {
   ...
   init(Id,total) {
     this.visible = true;
     this.$nextTick(() => {
         getEnterprisesById(Id).then(data => {
           console.log(data)
           // this.data = data.data;
           // this.txtVal = data.data.intro.length;
           this.formData3=data.data.entrepreneur;
         });
     });
   },
