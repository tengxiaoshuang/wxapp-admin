  ...
  <el-dialog :visible.sync="visible" :show-close="false" width="90%" :modal="true" :close-on-click-modal="false" :close-on-press-escape="false">
    ...
    <el-table border :data="formData" style="width: 100%;margin-left:20px">
      ...
      <el-table-column label="头像">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <el-image :src="scope.row.cardFiles"></el-image>
      ...
      <el-table-column label="名片">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <el-image :src="scope.row.avatarFiles"></el-image>
            <div slot="reference" class="name-wrapper"><el-tag size="medium">名片</el-tag></div>
          </el-popover>
        </template>
      </el-table-column>
    ...
    <el-table border :data="data.educationalList" style="width: 100%;margin-left:20px">
        ...
        if (Id) {
          getListById(Id).then(data => {
            ...
            this.data.educationalList = data.data.educationalList.map(x => ({
              ...x,
              startTimeEd: moment(x.startTime).format('YYYY-MM-DD'),
              endTimeEd: moment(x.endTime).format('YYYY-MM-DD')
            }));
